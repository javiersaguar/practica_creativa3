...
def main(base_path):
  ...
  try:
    ...
    spark = pyspark.sql.SparkSession(sc).builder       .appName(APP_NAME)       .config("spark.sql.extensions", "org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions")       .config("spark.sql.catalog.minio", "org.apache.iceberg.spark.SparkCatalog")       .config("spark.sql.catalog.minio.type", "hadoop")       .config("spark.sql.catalog.minio.warehouse", "s3a://flight-data/warehouse")       .config("spark.hadoop.fs.s3a.endpoint", "http://localhost:9000")       .config("spark.hadoop.fs.s3a.access.key", "minioadmin")       .config("spark.hadoop.fs.s3a.secret.key", "minioadmin")       .config("spark.hadoop.fs.s3a.path.style.access", "true")       .config("spark.hadoop.fs.s3a.impl", "org.apache.hadoop.fs.s3a.S3AFileSystem")       .getOrCreate()
  ...
  with mlflow.start_run():
    ...
    mlflow.log_param("data_source", "iceberg/minio")
    mlflow.log_metric("training_records", features.count())
  ...
  for column in ["Carrier", "Origin", "Dest", "Route"]:
    ...
    string_indexer_output_path = "s3a://flight-data/models/string_indexer_model_{}.bin".format(
      column
    )

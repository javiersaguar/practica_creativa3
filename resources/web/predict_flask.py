...
# Métrica personalizada: predicciones por categoría
from prometheus_client import Counter
flight_predictions_total = Counter(
...
flight_requests_total = Counter(
    ...
    'Total de solicitudes de predicción enviadas a Kafka'
)
...
def get_producer():
    ...
    if _producer is None:
        _producer = KafkaProducer(bootstrap_servers=[os.environ.get("KAFKA_BOOTSTRAP_SERVERS", "localhost:9092")])
    return _producer
...
def classify_flight_delays_realtime():
  ...
  get_producer().send(PREDICTION_TOPIC, message_bytes)
  flight_requests_total.inc()
